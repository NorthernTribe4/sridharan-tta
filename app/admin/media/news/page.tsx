...
export default async function AdminNewsPage() {
  const { supabase, user } = await requireAdmin("/admin/media/news")
  ...
  const articles: NewsArticle[] = (data as NewsArticle[] | null) ?? []
    ...
    <>
      <div className="bg-[#0A0A0B] border-b border-[#27272A] text-white py-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          <div>
            ...
            <h1 className="font-heading font-extrabold text-3xl mt-1 text-white">News Articles</h1>
          </div>
      ...
      <div className="bg-[#0A0A0B] min-h-screen">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <NewsAdminClient articles={articles} />
        </div>
      </div>
    </>
  )
